  			...
  			<div class="sea-box" style="margin-top:10px;">
	  			<div class="shengfen shengfen03" >
					<div class="span pp"> 
                    	...
                    	<input class="startTime" type="text"  style="width:100px;height:30px;font-size: 14px;padding-bottom:0px;" onFocus="WdatePicker({isShowClear:false,readOnly:true})"/>
                    </div>	 
					<div class="span pp">
                    	<span  style="width:80px;height: 32px;line-height: 24px;    line-height: 30px;">结束时间</span>   
                    	<input class="endTime"  type="text"  style="width:100px;height: 30px;font-size: 14px;padding-bottom:0px;" onFocus="WdatePicker({isShowClear:false,readOnly:true})"/>
                    </div>
					<input type="text" value="" placeholder="请输入要搜索的文字" class="input01"  style="width:300px;height: 30px;line-height: 24px;font-size: 16px;">
	    ...
	    $.each(data, function(key, value) {
	    	if( key>=(shu-1)*num_value && key< shu*num_value){
				if(key%2!=0){
					...
					+"	    <td>"+value.grade+"</td>"
					+"	    <td>"+value.strTime+"</td>"
					+"	    <td><a href='/DataShare/manager/earthQuakeSupplyDataToJsp.action?earthQuakeId="+value.id+"'>在线整理</a></td>"
					+"    </tr>"
				}else{ 
					str=str+"<tr>"
					+"	    <td>"+value.year+"</td>"
