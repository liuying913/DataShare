  ...
  <script>
    ...
    var isManager = $("#isManager").val();
  	$(document).ready(function(){
		...
		var url="/DataShare/queryApplyUserList.action?isManager="+isManager+"&applyState="+applyState;
  		...
  		$(".seabtn").click(function(){
  			...
  			var url="/DataShare/queryApplyUserList.action?isManager="+isManager+"&applyState="+applyState+"&selectParams="+item;
  			ajaxRequest(url,"get",function(res){
				data = res;
				shua();
				page();
	    ...
	    $.each(data, function(key, value) {
	    	if( key>=(shu-1)*num_value && key< shu*num_value    ){
				if(key%2!=0){
					...
					+"	    <td><a href='/DataShare/applyListToPage1ToJsp.action?isManager="+isManager+"&applyId="+value.id+"&liuCheng="+value.liuCheng+"'>查看</a></td>"
					+"    </tr>"
				}
				else{ 
					str=str+"<tr>"
