	...
	$('.shengfen-button').click(function() {
		...
		for(var i = 0; i < $(".input-warp02").eq(This).find('input:checked').length; i++) {taizhang[i] = $(".input-warp02").eq(This).find('input:checked').eq(i).val();}	
			...
			$.ajax({
				...
				setTimeout('parent.$("#iframe").css("height",$("#iframeheight").height());',700);			
						...
						function (ec) {
							...
							var option = {
								...
								dataRange: {
							        ...
							        max : 2000,
							        splitList: [
					                    {start: 3, label: '大于2次', color: '#00F5FF'},
					                    {start: 2, end: 2, label: '2次', color: '#C0FF3E'},
					                    {start: 1, end: 1, label: '1次', color: '#7FFF00'},
