	...
	$('.shengfen-button').click(function() {
		...
		for(var i = 0; i < $(".input-warp02").eq(This).find('input:checked').length; i++) {taizhang[i] = $(".input-warp02").eq(This).find('input:checked').eq(i).val();}	
			...
			$.ajax({
				...
				//for(j=1;j<=dui;j++){hy+='<th data-i="'+j+'"></th>';}
				for(j=1;j<=12;j++){
					
					if(j==1||j==3||j==4||j==7||j==8||j==9||j==10||j==12){
						d=31;
					}
					else if(j==2){
						if(dui==365){
						...
						$.each(value, function(i, item) {
							if(j==i){
								...
								if(parseInt(item.couNum)>=1 && parseInt(item.couNum)<=2){//1到2
									hy+="<td class='green' title="+item.couNum+"></td>";
								}else if(parseInt(item.couNum)>=3 && parseInt(item.couNum)<=4){//3到4
									hy+="<td class='yellow' title="+item.couNum+"></td>";
								}else if(parseInt(item.couNum)>=5){
									hy+="<td class='red' title="+item.couNum+"></td>";
								}else{
									hy+="<td  title="+item.couNum+"></td>";		
								}
								return true;
							}
						});
						//if(ty=="0"){hy+="<td style='background-color: #E5ED64;'>缺失</td>";	}
					 }
					hy+="</tr>";
				});	
				hy+="</tbody>";
