$(document).ready(function() {
	...
	$(".warp-button .button").click(function(){
		...
		var url="/DataShare/analysisExcel/s30S/dep.action?userId="+userId+"&startTime="+kaishi+"&endTime="+jieshu+"&siteType="+siteType+"&fileType="+fileType;
	...
	function getAjax(userId,kaishi,jieshu,siteType,fileType){
		url="/DataShare/analysis/s30S/departmentDownAnaly.action?userId="+userId+"&startTime="+kaishi+"&endTime="+jieshu+"&siteType="+siteType+"&fileType="+fileType;
		$.ajax({
			url: url,
			type: "get",
			data: "json",
			...
			var SuccessCallback=function(){
				...
				var UArrayV=[];	
				var UArrayT=[];	
				var tyr='<table class="tftable" border="1" id="project"><tbody><tr><th>部委名称</th><th>总下载数量</th><th>平均下载数量</th></tr>';							 		
				...
				require(
					...
					function (ec) {
						...
						var optionleft = {
							color: ['#306bb5'],
							title: {x: 'center', y: 'top', text: '各部委总下载数量'},						
							tooltip: {},
							xAxis: {																		
			...
			　　　　　　　　　　barWidth:55,//设置柱的宽度，要是数据太少，柱子太宽不美观~
							   data: UArrayV
							}]
						};						
				...
				require(
					...
					function (ec) {
						...
						var optionright = {
							color: ['#B5C334'],
							title: {x: 'center', y: 'top', text: '各部委平均下载数量'},	
							tooltip: {
								trigger: 'axis'
							},
							toolbox: {
