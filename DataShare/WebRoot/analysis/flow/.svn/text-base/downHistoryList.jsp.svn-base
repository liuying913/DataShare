...
<body>
  ...
  <div id="iframeheight">
  		<div class="pbox tspbox">
			<!--<div class="frame_title"><span><i class="icon-bookmark"></i>&nbsp;${applyTitle}</span></div>-->
  ...
  <script>
     $.ajaxSetup({ cache: false });//全局禁用缓存
  	$(document).ready(function(){
  		var str="";
  		ajaxRequest("/DataShare/analysis/s30S/downHistoryList.action?siteType=2&fileType=2","get",function(res){
			data = res;
			shua();
			page();
  		})
  		$(".seabtn").click(function(){
  			...
  			if(item==""){
	  			...
	  			ajaxRequest("/DataShare/analysis/s30S/downHistoryList.action?siteType=2&fileType=2&fileParams="+item,"get",function(res){
					data = res;
		            shua();
					page();
		  		})
