...
$(document).ready(function() {
	...
	$('#selectData').click(function() {
		var njrNumber = 0;
		var This = $('#selectData').index($(this));
		var Datetype = $('.Datetype').eq(This).val(); //数据第几块
		$('.warp02-bottom').eq(This).slideDown(); //提交--显示
		$('.tr-01').eq(This).nextAll().remove();
		$('.ul-img').eq(This).html('');
		...
		$.ajax({
			...
			for(var p in data){
				...
				//右侧  默认显示第一天的表格
				fillLeftData(This,yearDayFirst,Datetype);
				//setTimeout('parent.$("#iframe").css("height",$("#iframeheight").height());',600);
	...
	$('.button').click(function() {
		...
		var params = getParams(This)+"&applyId="+applyId;
		$.ajax({
			...
			setTimeout('$(".zbg", window.parent.document).hide();$(".mytips", window.parent.document).hide();',300);
		})
	})	
