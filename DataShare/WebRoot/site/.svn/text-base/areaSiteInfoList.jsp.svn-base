...
<body>
  ...
  <div id="iframeheight" style="height: 459px;">
  		<div class="pbox tspbox">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table01">
              <tr>
                <th width="10%">流动站编号</th>
                <th width="10%">流动站名称</th>
                <th width="12%">纬度</th>
                <th width="17%">经度</th>
  ...
  <script>		
  	...
  	$(document).ready(function(){
  		var str="";
  		ajaxRequest("/DataShare/getSiteInfoList.action?siteType=2","get",function(res){
			data = res;
			shua();
			page();
  		})
  		$(".seabtn").click(function(){
  			...
  			ajaxRequest("/DataShare/getSiteInfoList.action?siteType=2&userParam="+item,"get",function(res){
				data = res;
	            shua();
				page();
	  		});
	    ...
	    $.each(data, function(key, value) {
	    	if( key>=(shu-1)*num_value && key< shu*num_value    ){
				if(key%2!=0){
					...
					+"	    <td>"+value.siteLat+"</td>"
					+"	    <td>"+value.siteLng+"</td>"
					+"	    <td><a href='/DataShare/showBaseSiteInfoToJsp.action?siteType=2&siteNumber="+value.siteNumber+"'>查看</a>&nbsp;<a href='/DataShare/updateAreaSiteInfoToJsp.action?siteNumber="+value.siteNumber+"'>编辑</a></td>"
					+"    </tr>"
				}
				else{
					str=str+"<tr>"
