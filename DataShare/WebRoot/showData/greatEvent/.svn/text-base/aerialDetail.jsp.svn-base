    ...
    <script>		$.ajaxSetup({ cache: false });//全局禁用缓存
    	...
    	$(document).ready(function(){
			...
			ajaxRequest("/DataShare/greatEvent/typelList.action?id="+id,"get",function(res){
				...
				$(".input01").eq(5).val(res[0].height_new);
				$(".input01").eq(4).val(res[0].height_old);
				
				$(".input01").eq(7).val(res[0].model_new);
				$(".input01").eq(6).val(res[0].model_old);
				$(".input01").eq(9).val(res[0].rinex_new);
				$(".input01").eq(8).val(res[0].rinex_old);
				
				$(".input01").eq(11).val(res[0].sn_new);
				$(".input01").eq(10).val(res[0].sn_old);
