         ...
         <div class="pbox tspbox" style="padding-bottom:0px;">
  		...
  		<div class="pbox">
            ...
            <div class="warp-button" style="text-align: center">
                ...
                <button type="button" id="back" class="shengfen-button" style="margin-left: 15px;width:111px;height: 26px;float: none;display: inline-block">返回</button>
            </div>
        </div>
        </div>
    <script>		$.ajaxSetup({ cache: false });//全局禁用缓存
    	...
    	$(document).ready(function(){
			...
			ajaxRequest("/DataShare/greatEvent/typelList.action?id="+id,"get",function(res){
				...
				$(".input01").eq(4).val(res[0].sn_new);
				$(".input01").eq(5).val(res[0].sn_old);
				$(".input01").eq(6).val(res[0].fw_new);
				$(".input01").eq(7).val(res[0].fw_old);
