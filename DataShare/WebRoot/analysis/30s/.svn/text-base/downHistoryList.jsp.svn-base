...
<body>
  ...
  <script> 	$.ajaxSetup({ cache: false });//全局禁用缓存
  	$(document).ready(function(){
  		var str="";
  		ajaxRequest("/DataShare/analysis/s30S/downHistoryList.action?siteType=1&fileType=1","get",function(res){
			data = res;
			shua();
			page();
  		})
  		$(".seabtn").click(function(){
  			...
  			if(item==""){
	  			...
	  			ajaxRequest("/DataShare/analysis/s30S/downHistoryList.action?siteType=1&fileType=1&fileParams="+item,"get",function(res){
					data = res;
		            shua();
					page();
		  		})
