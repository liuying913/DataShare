	$(document).ready(function() {
			...
			str = "";
			ajaxRequest("/DataShare/user/downHistoryList.action?applyId=" + applyId, "get", function(res) {
				var JSONObj = res;
			...
			$(".seabtn").click(function() {
				...
				ajaxRequest("/DataShare/user/downHistoryList.action?applyId=" + applyId + "&fileParams=" + item,"get", function(res) {
					var JSONObj = res;
					currentPage = JSONObj[0].currentPage;
					pageCount = JSONObj[0].pageCount;
					recordCount = JSONObj[0].recordCount;
			...
			$("#exportUserHistoryExcel").click(function() {
				var item = $(".sea-box .input01").val();
				var url="/DataShare/excel/exportUserHistoryExcel.action?applyId=" + applyId + "&fileParams=" + item;
		...
		for ( var key = 1; key <= data.length; key++) {
			...
			if (key % 2 != 0) {
				str = str + "<tr class='even'>" 
						...
						"	    <td>" + data[key - 1].site_number + "</td>" 
						+ "	    <td>" + data[key - 1].fileyear + "/" + data[key - 1].fileDayyear+ "</td>" 
						+ "	    <td>" + data[key - 1].apply_unit+ "</td>"
						+ "	    <td>" + data[key - 1].apply_person+ "</td>" 
						+ "	    <td>" + data[key - 1].downTimeStr+ "</td>" 
						+ "	    <td>" + data[key - 1].fileComp + "</td>"
						+ "    </tr>"
			} else {
				str = str + "<tr>" 
						+ "	    <td>" + data[key - 1].fileName + "</td>"
	...
	function searchpage(currentPage) {
		ajaxRequest("/DataShare/user/downHistoryList.action?applyId="
				+ $("#applyId").val() + "&currentPage=" + currentPage, "get",
				function(res) {
					var JSONObj = res;
					currentPage = JSONObj[0].currentPage;
					pageCount = JSONObj[0].pageCount;
		...
		$(".seabtn").click(
				function() {
					var item = $(".sea-box .input01").val();
					ajaxRequest(
						"/DataShare/user/downHistoryList.action?applyId="
								+ applyId + "&fileParams=" + item
