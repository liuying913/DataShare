  ...
  <script>
    $.ajaxSetup({ cache: false });//全局禁用缓存
  	$(document).ready(function(){
  		...
  		ajaxRequest("/DataShare/earthQuake/waitCheckListForUser.action?state=4&siteType=1","get",function(res){
			data = res;
			shua();
			page();
  		})
  		$(".seabtn").click(function(){
			...
			//alert( state );
  			ajaxRequest("/DataShare/earthQuake/waitCheckListForUser.action?state=4&siteType=1&userParam="+item,"get",function(res){
				data = res;
		        shua();
				page();
	  		})
  	...
  	function shua(){
	    ...
	    $.each(data, function(key, value) {
	    	if( key>=(shu-1)*num_value && key< shu*num_value){
	    		...
	    		var operation="	    <td><a href='/DataShare/earthQuake/queryCheckToJsp.action?noCheckFlag=false&id="+value.id+"&applyTitle=应急数据申请查看'>查看</a></td>";
