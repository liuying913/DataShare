...
<body>
  		...
  		<div class="pbox tspbox">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table01">
              <tr>
                ...
                <th style="white-space: nowrap;">所属地区</th>
                <th style="white-space: nowrap;">承建部委</th>
  ...
  <script>		$.ajaxSetup({ cache: false });//全局禁用缓存
  	$(document).ready(function(){
  		var str="";
  		ajaxRequest("/DataShare/getSiteInfoList.action?siteType=1","get",function(res){
			data = res;
			shua();
			page();
  		})
  		$(".seabtn").click(function(){
  			...
  			ajaxRequest("/DataShare/getSiteInfoList.action?siteType=1&userParam="+item,"get",function(res){
				data = res;
	            shua();
				page();
	  		});
	    ...
	    $.each(data, function(key, value) {
	    	if( key>=(shu-1)*num_value && key< shu*num_value    ){
				if(key%2!=0){
					...
					+"	    <td><a href='/DataShare/showBaseSiteInfoToJsp.action?siteType=1&siteNumber="+value.siteNumber+"'>查看</a>&nbsp;<a href='/DataShare/updateBaseSiteInfoToJsp.action?siteType=1&siteNumber="+value.siteNumber+"'>编辑</a></td>"
					+"    </tr>"
				}
				else{
					str=str+"<tr>"
